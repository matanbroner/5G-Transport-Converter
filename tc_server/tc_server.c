...
int handle_packet(void * data, int len, char* err, int err_buf_len) {
    ...
    char* proxy_key = gen_proxy_key(iph->saddr, tcph->sport, iph->daddr, tcph->dport);
    if (conn_key == NULL) {
    ...
    struct proxy_t *proxied = ogs_hash_get(proxy_hash, conn_key);
    if (proxied != NULL) {
...
int handle_tlvs(struct convert_opts *opts) {
